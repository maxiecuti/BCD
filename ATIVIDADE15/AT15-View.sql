...
-- atividade 11
ALTER VIEW vw_clientes_aniversariantes AS
SELECT nome_cliente, telefone_cliente, MONTH(data_nascimento) AS mes_aniversariantes
FROM tb_cliente
...
-- atividade 14
SELECT nome_cliente,
CASE mes_aniversariantes
 WHEN 1 THEN 'Janeiro'
 WHEN 3 THEN 'Março'
 WHEN 5 THEN 'Maio'
END AS mes_aniversariantes
FROM vw_clientes_aniversariantes
WHERE mes_aniversariantes IN (1, 3, 5)
ORDER BY mes_aniversariantes;

select * from vw_clientes_aniversariantes;

-- atividade 15
SELECT vw_clientes_mais_lucrativos.nome_cliente, vw_produtos_lucro.lucro_bruto
FROM vw_produtos_lucro
JOIN vw_clientes_mais_lucrativos
WHERE vw_produtos_lucro.categoria = "alimentos";
